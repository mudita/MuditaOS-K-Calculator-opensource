...
fun isCalculationError(): Boolean {
    return division_by_0 || domain_error || syntax_error || is_infinity || require_real_number
}

class Calculator(private val numberPrecision: Int = 10) {
    ...
    private fun gammaLanczos(x: BigDecimal): BigDecimal {
        ...
        val g = BigDecimal(7)
        if (xx < BigDecimal(0.5)) return (Math.PI / (sin(Math.PI * xx.toDouble()) * gammaLanczos(
            BigDecimal(1.0 - xx.toDouble())
        ).toDouble())).toBigDecimal()
        xx--
        var a = p[0]
        val t = xx + g + BigDecimal(0.5)
        for (i in 1 until p.size) a += p[i] / (xx.toDouble() + i)
        return (sqrt(2.0 * Math.PI) * t.toDouble()
            .pow(xx.toInt() + 0.5) * exp(-t.toDouble()) * a).toBigDecimal()
    }
